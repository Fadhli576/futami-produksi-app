...
@section('content')
    @if (Auth::user()->role_id == 3)
        <div class="card p-3 mb-5 shadow-sm">
            <h5>Jenis karton</h5>
            <form class="row" action="{{ route('jenis-karton-store') }}" method="POST">
                @csrf
                <div class="col-12 col-md-6">
                    <label for="">Jenis karton</label>
                    <input class="form-control" type="text" name="name" id="" placeholder="Jenis karton">
                </div>
                <div class="col-12 col-md-6">
    ...
    <table
        ...
        @forelse ($kartons as $karton)
            <tr class="bg-white">
                <td>{{ $loop->iteration }}</td>
                <td>{{ $karton->name }}</td>
                <td>{{ $karton->spesifikasi }}</td>
                <td>
                    <form action="{{ route('jenis-karton-delete', $karton->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <a class="btn" style="color:#98c1d9" href="{{ route('jenis-karton-edit', $karton->id) }}"><i
   ...
   {{-- {{ $kartons->links('pagination::bootstrap-4') }} --}}
@endsection
