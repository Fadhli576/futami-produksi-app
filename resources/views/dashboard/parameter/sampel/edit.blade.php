...
@section('content')
    @if (Auth::user()->role_id == 3)
        <div class="card p-3 mb-5 shadow-sm">
            <h5>Parameter Sampel</h5>
            <form class="row" action="{{ route('parameter-sampel-update', $parameterSampel->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="col-12">
                    <label for="">Name</label>
                    <input class="form-control" type="text" name="name" id="" placeholder="Parameter Sampel" value="{{ $parameterSampel->name }}">
                </div>
    ...
    {{-- <table
        ...
        @forelse ($parameter_sampel as $sampel)
            <tr class="bg-white">
                <td>{{ $loop->iteration }}</td>
                <td>{{ $sampel->name }}</td>
                <td>{{ $sampel->spesifikasi }}</td>
                <td>
                    <form action="{{ route('parameter-sampel-delete', $sampel->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <a class="btn" style="color:#98c1d9" href="{{ route('parameter-sampel-edit', $sampel->id) }}"><i
