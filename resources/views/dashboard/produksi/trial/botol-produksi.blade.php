...
@section('content')
    <div class="card p-3 mb-5 shadow-sm">
        <h5>Botol</h5>
        <form class="row" action="{{ route('trial-botol-store', ['produksi_id' => $produksi_id, 'batch_id' => $batch_id]) }}"
            method="POST">
            @csrf
            <div class="col-12">
                <label for="">Trial Botol</label>
                <input class="form-control" type="number" name="trial_botol" id="" placeholder="Trial">
            </div>
