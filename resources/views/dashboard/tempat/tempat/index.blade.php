...
@section('content')
    @if (Auth::user()->role_id == 3)
        <div class="card p-3 mb-5 shadow-sm">
            <h5>Tempat</h5>
            <form class="row" action="{{ route('tempat-store') }}" method="POST">
                @csrf
                <div class="col-12">
                    <label for="">Name</label>
                    <input class="form-control" type="text" name="name" id="" placeholder="Tempat">
                </div>
    ...
    <table
        ...
        @forelse ($tempat as $tempat)
            <tr class="bg-white">
                ...
                <td>{{ $tempat->name }}</td>
                <td>
                    <form action="{{ route('tempat-delete', $tempat->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <a class="btn" style="color:#98c1d9" href="{{ route('tempat-edit', $tempat->id) }}"><i
